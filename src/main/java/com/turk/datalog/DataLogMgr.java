...
public class DataLogMgr
{
	...
	public synchronized void addFtpLog(DataFtpLogInfo info)
	{
		...
		{
			this.logger.error("提交数据库日志时异常，" + this.ftplogs.size() + "条日志未提交。", e);
			return;
		}
		finally
		{
	...
	private void sqlldrCommit()
	{
		try
		{
			...
			this.logger.debug("数据库日志提交完毕，返回码为: " + code);
		...
		{
			this.logger.error("提交数据库日志时异常", e);
			return;
		}
		finally
		{
	...
	private void jdbcCommit()
	{
		...
		{
			this.logger.error("提交数据库日志时异常，" + this.logs.size() + "条日志未提交。", e);
			return;
		}
		finally
		{
