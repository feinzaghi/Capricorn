...
private static Logger log = LogMgr.getInstance().getSystemLogger();
	...
	private static TaurusDayComplate _instance = null;
	
	private Map<String,CdrFileDayInfo> _fileInfoMap = new HashMap();
	
	public static TaurusDayComplate getInstance()
	{
		if(_instance == null)
			_instance = new TaurusDayComplate();
		return _instance;
	}
	
	public TaurusDayComplate()
	{
	...
	public void ExcuteScan()
	{
		...
		try
		{
			...
			strSql = "select start_time,city_id,avg(case when filerate > 100 then 100 else filerate end) filerate from ("
				...
				+" 	               trunc(to_date(data_time, 'yyyymmddhh24miss'), 'DD') as start_time,"
				+" 	               count(*) as filenum"
				+" 	          from etl_mod_ea_log a,cfg_city b"
				+" 	         where a.city_id = b.city_id and b.vendor = 'ZY0808'"
				+" 	           and datatype = 'CDR'"
				+" 	           and a.ea_time > sysdate - 20"
				+" 	         group by a.city_id, bscid, trunc(to_date(data_time, 'yyyymmddhh24miss'), 'DD'))"
	...
	private boolean CreateIndexFile(String strDate,int nCityID)
	{
		...
		try
		{
		    ...
		    code = ftp.uploadFile(file1XV1.getAbsolutePath(), docName);
		    switch(code)
		    {
		    	case 100://成功
